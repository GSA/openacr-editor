<script>
  ...
  $: currentEvaluationCriteria =
    $evaluation["chapters"] && $evaluation["chapters"][chapterId]["criteria"]
      ? $evaluation["chapters"][chapterId]["criteria"].find(
          ({ num }) => num === criteria,
        )
      : null;
  $: currentEvaluationComponent = currentEvaluationCriteria
    ? currentEvaluationCriteria.components.find(
        ({ name }) => name === component,
      )
    : null;
  $: currentEvaluationCriteriaIndex =
    $evaluation["chapters"] && $evaluation["chapters"][chapterId]["criteria"]
      ? $evaluation["chapters"][chapterId]["criteria"].findIndex(
          ({ num }) => num === criteria,
        )
      : null;
  $: currentEvaluationComponentIndex = currentEvaluationCriteria
    ? currentEvaluationCriteria.components.findIndex(
        ({ name }) => name === component,
      )
    : null;
  $: notesCharCount = $evaluation["chapters"][chapterId]["criteria"][
    currentEvaluationCriteriaIndex
  ]["components"][currentEvaluationComponentIndex]["adherence"]["notes"]
    ? $evaluation["chapters"][chapterId]["criteria"][
        currentEvaluationCriteriaIndex
      ]["components"][currentEvaluationComponentIndex]["adherence"]["notes"]
        .length
    : 0;
  ...
  function showNotesMessage(e) {
    const messageBox = document.getElementById(
      `evaluation-${criteria}-${component}-notes-message`,
    );
    if (notesCharCount > 50) {
      messageBox.innerHTML = "<span class='notes-good'>Good&nbsp;</span>";
    } else if (notesCharCount > 1) {
      messageBox.innerHTML =
        "<span class='notes-need-more'>Longer description may be helpful&nbsp;</span>";
    } else {
      messageBox.innerHTML = "";
    }
  }
...
{#if currentComponent.label}
  <HeaderWithAnchor id="{criteria}-{component}" level="3"
    >{currentComponent.label}</HeaderWithAnchor
  >
{:else}
  <br /><br />
{/if}
...
{#if currentEvaluationCriteria}
  {#if currentEvaluationComponent}
    <div class="field">
      ...
      <select
        ...
        bind:value={
          $evaluation["chapters"][chapterId]["criteria"][
            currentEvaluationCriteriaIndex
          ]["components"][currentEvaluationComponentIndex]["adherence"]["level"]
        }
        on:blur={() => {
      ...
      >
        <option
          name="option-evaluation-{criteria}-{component}-level-none"
          value="">- Select -</option
        >
        {#each terms as term}
          <option
            name="option-evaluation-{criteria}-{component}-level-{term.id}"
            value={term.id}>{term.label}</option
          >
        {/each}
      </select>
    ...
    <div class="field">
      <label for="evaluation-{criteria}-{component}-notes"
        >Remarks and Explanations</label
      >
      <div
        id="evaluation-{criteria}-{component}-notes-message"
        class="notes-message"
        aria-live="polite"
      ></div>
      <textarea
  ...
  {:else}
    <p>
      Could not find component '{component}' for critera '{criteria}' in '{chapterId}'.
    </p>
  {/if}
{:else}
  <p>
