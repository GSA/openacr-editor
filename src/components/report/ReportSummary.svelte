...
{#if $evaluation["repository"]}
  ...
  <a href="{$evaluation["repository"]}" target="_blank">{$evaluation["repository"]}
...
{#if $evaluation["feedback"]}
  ...
  <a href="{$evaluation["feedback"]}" target="_blank">{$evaluation["feedback"]}
    <span class="visuallyhidden">(opens in a new window)</span></a>
{/if}
{#if $evaluation["related_openacrs"] && $evaluation["related_openacrs"].length > 0}
  ...
  <ul>
    {#each $evaluation["related_openacrs"] as related}
      {#if related.url}
        <li><a href="{related.url}" target="_blank">{related.url} ({related.type})
          <span class="visuallyhidden">(opens in a new window)</span></a></li>
      {/if}
    {/each}
  </ul>
{/if}
